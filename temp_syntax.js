        ...
        // Show professional lockout screen with real data
        function showRealLockoutScreen(lockoutData) {
            document.body.innerHTML = `
                <div style="background: linear-gradient(135deg, #8B0000 0%, #DC143C 50%, #8B0000 100%); color: white; min-height: 100vh; display: flex; align-items: center; justify-content: center; font-family: Arial, sans-serif;">
                    <div style="text-align: center; max-width: 600px; padding: 40px; background: rgba(0,0,0,0.3); border-radius: 10px; border: 2px solid #ff0000;">
                        <div style="font-size: 4rem; margin-bottom: 20px;">🚨</div>
                        <h1 style="font-size: 3rem; margin-bottom: 30px; text-shadow: 2px 2px 4px rgba(0,0,0,0.8);">ACCOUNT LOCKED</h1>
                        <div style="background: rgba(139,0,0,0.5); padding: 20px; border-radius: 8px; margin-bottom: 20px;">
                            <h2 style="color: #ffcccc; margin-bottom: 15px;">Security Violation Detected</h2>
                            <div style="text-align: left; font-size: 1.1rem; line-height: 1.8;">
                                <p><strong>User:</strong> ${lockoutData.username}</p>
                                <p><strong>Reason:</strong> ${lockoutData.reason}</p>
                                <p><strong>Time:</strong> ${new Date(lockoutData.timestamp).toLocaleString()}</p>
                                <p><strong>Status:</strong> <span style="color: #ff6666;">PERMANENTLY LOCKED</span></p>
                            </div>
                        </div>
                        <div style="font-size: 0.9rem; color: #ffcccc; line-height: 1.6;">
                            <p>Your account has been automatically locked due to suspicious gaming activity detected by Dynatrace's fraud detection system.</p>
                            <p><strong>This action is irreversible.</strong> Contact the casino administrator for appeals.</p>
                        </div>
                    </div>
                </div>
            `;
        ...
        socket.on('user-lockout', (lockoutInfo) => {
            ...
            if (lockoutInfo.username === currentUser) {
                console.log(`🔒 Current user ${currentUser} has been locked out - showing lockout immediately`);
                showDynamicLockout(lockoutInfo);
            }
        });
        ...
        // Periodic lockout check (every 10 seconds) for real-time detection
        setInterval(() => {
            checkCurrentUserLockout();
        }, 10000);

        // Quick lockout check function for periodic use
        async function checkCurrentUserLockout() {
            try {
                const currentUser = localStorage.getItem('vegasUsername') || 'Lawro';
                ...
                if (data.locked && !window.isLockoutScreenShowing) {
                    console.log('🚨 Periodic check detected lockout - showing screen immediately');
                    showDynamicLockout(data);
                }
            } catch (error) {
        ...
        // Dynamic lockout that appears instantly without refresh
        function showDynamicLockout(lockoutData) {
            console.log('🚨 Showing INSTANT lockout screen for:', lockoutData);
            window.isLockoutScreenShowing = true;
            
            // Create overlay that appears on top of current content
            ...
            lockoutOverlay.style.cssText = `
                ...
                z-index: 10000;
                animation: lockoutAppear 0.5s ease-in;
            `;
            ...
            lockoutOverlay.innerHTML = `
                ...
                <div style="text-align: center; max-width: 600px; padding: 40px; background: rgba(0,0,0,0.4); border-radius: 15px; border: 3px solid #ff0000; box-shadow: 0 0 50px rgba(255,0,0,0.5);">
                    <div style="font-size: 5rem; margin-bottom: 20px; animation: pulse 1s infinite;">🚨</div>
                    <h1 style="font-size: 3.5rem; margin-bottom: 30px; text-shadow: 3px 3px 6px rgba(0,0,0,0.8); color: #ff4444;">ACCOUNT LOCKED</h1>
                    <div style="background: rgba(139,0,0,0.6); padding: 25px; border-radius: 10px; margin-bottom: 25px; border: 2px solid #ff4444;">
                        <h2 style="color: #ffcccc; margin-bottom: 20px; font-size: 1.3rem;">⚠️ REAL-TIME SECURITY ENFORCEMENT ⚠️</h2>
                    ...
                    <div style="font-size: 1rem; color: #ffdddd; line-height: 1.8; margin-bottom: 20px;">
                        <p>🔒 <strong>Your session has been terminated immediately</strong></p>
                        <p>This real-time lockout was triggered by Dynatrace's advanced fraud detection system.</p>
                        <p><strong>No browser refresh required - enforcement is instant.</strong></p>
                    </div>
            ...
            // Add to page - appears instantly over current content
            document.body.appendChild(lockoutOverlay);
            
            // Disable all interaction with underlying page
